...
* `run-auen.swift`: Parallel Swift script to perform parameter sweep concurrently, similar to `run-one-sweep.py`.  Example usage: `swift-t -n PROCS run-one-sweep.swift`.  Runs _PROCS-1_ tasks at a time (one process is reserved for Swift.
